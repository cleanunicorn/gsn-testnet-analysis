...
def load_blocks() -> List[Dict[str, Any]]:
    ...
    end_time = time.time()
    print(f"Loaded {len(blocks)} blocks ({total_size / (1024*1024):.2f} MB) in {end_time - start_time:.2f} seconds")
    return blocks

def calculate_gas_fees(blocks: List[Dict[str, Any]]) -> Tuple[List[int], List[Dict[str, Any]]]:
        ...
        for tx in block.get('transactions', []):
            ...
            if 'gasPrice' in tx:
                try:
                    ...
                    print(f"Warning: Invalid gas price in transaction {tx.get('hash', 'unknown')}: {e}")
                    continue
    
    end_time = time.time()
    print(f"Processed {total_txs} total transactions, found {len(transactions)} valid transactions in {end_time - start_time:.2f} seconds")
...
def calculate_ema(gas_fees: List[int], alpha: float = 0.1) -> List[float]:
    ...
    end_time = time.time()
    print(f"Calculated EMA for {len(gas_fees)} values in {end_time - start_time:.2f} seconds")
    return ema_values
...
def find_anomalies_with_ema(transactions: List[Dict[str, Any]], window_size: int = 100, threshold_multiplier: float = 2.0) -> List[Dict[str, Any]]:
    ...
    sort_end = time.time()
    print(f"Sorted {len(transactions)} transactions in {sort_end - sort_start:.2f} seconds")
        ...
        if tx['gas_price'] > threshold:
            anomalies.append({
                **tx,
                ...
                'deviation_percentage': ((tx['gas_price'] - current_ema) / current_ema) * 100 if current_ema > 0 else float('inf')
            })
    
    anomaly_end = time.time()
    print(f"Found {len(anomalies)} anomalies in {anomaly_end - anomaly_start:.2f} seconds")
...
def main():
    try:
        ...
        anomalies = find_anomalies_with_ema(transactions, window_size, threshold_multiplier)
        ...
        if anomalies:
            ...
            print(f"Average gas fee of anomalies: {statistics.mean(anomaly_gas_fees):.2f}")
            print(f"Median gas fee of anomalies: {statistics.median(anomaly_gas_fees)}")
            print(f"Average deviation percentage: {statistics.mean([a['deviation_percentage'] for a in anomalies]):.2f}%")
